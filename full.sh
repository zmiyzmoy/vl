...
# ======================================================================
# Fully Automated VLESS Xray Installation with Dedicated User
# For Ubuntu 22.04
# ======================================================================
...
# Function to create a dedicated xray user
create_xray_user() {
    print_message "info" "Creating dedicated xray user..."
    
    # Check if user already exists
    if id "xray" &>/dev/null; then
        print_message "info" "User 'xray' already exists."
    else
        # Create user
        useradd -r -m -s /bin/false xray
        print_message "success" "User 'xray' created successfully."
    fi
...
configure_xray() {
    ...
    # Create a very simple configuration without any routing rules that could cause errors
...
# Function to configure Xray service with xray user
configure_xray_service() {
    print_message "info" "Configuring Xray service..."
    
    # Create the service file using the xray user
...
EOF
    ...
    # Remove any drop-in configuration files that might interfere
    ...
    if systemctl is-active --quiet xray; then
        ...
        # Debug output
        print_message "info" "Checking Xray configuration..."
        /usr/local/bin/xray -test -config /usr/local/etc/xray/config.json
        
        # Try with root user as fallback
        print_message "warning" "Trying with root user as fallback..."
        cat > /etc/systemd/system/xray.service << EOF
[Unit]
Description=Xray Service
Documentation=https://github.com/xtls
...
EOF
        ...
        if systemctl is-active --quiet xray; then
            print_message "success" "Xray service started successfully with root user."
        else
            print_message "error" "Failed to start Xray service even with root user."
            exit 1
        fi
    fi
}
...
print_summary() {
    ...
    echo "- Service running as: $(systemctl show -p User xray | cut -d= -f2) user"
    echo ""
...
# Main function
main() {
    print_message "info" "Starting fully automated VLESS Xray installation..."
